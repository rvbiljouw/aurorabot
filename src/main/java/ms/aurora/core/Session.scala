...
import ms.aurora.core.actor.{RandomSupervisor, SessionBridge, ScriptSupervisor}
...
class Session(group: ThreadGroup, ui: AppletWidget) extends Runnable {
  ...
  @BeanProperty var randomSupervisor: ActorRef = null
  ...
  override def run() {
    if (clientManager.start()) {
      Repository.set(clientManager.getApplet.hashCode, this)
      ui.setApplet(clientManager.getApplet)

      scriptSupervisor = actorSystem.actorOf(
        Props(classOf[ScriptSupervisor], this))
      randomSupervisor = actorSystem.actorOf(
        Props(classOf[RandomSupervisor], this))
