...
import ms.aurora.core.model.MapTile;
...
public class MapDataPacketHandler implements PacketHandler {
    ...
    public void handle(IncomingPacket incomingPacket) throws IOException {
        Logger.getLogger(MapDataPacketHandler.class).info("Receiving mapdata");
            ...
            for (int i = 0; i < len; i++) {
                ...
                for (int j = 0; j < sublen; j++) {
                    ...
                    if ((mask & (RSMap.BLOCKED | RSMap.INVALID)) != 0) {
                        ...
                        MapTile tile = MapTile.getAt(rx + i, ry + j);
                        if(tile != null) {
                            tile.setMask((byte)mask);
                            tile.update();
                        } else {
                            tile = new MapTile();
                            tile.setX(rx + i);
                            ...
                            tile.setMask((byte)mask);
                            tile.save();
                            Logger.getLogger(MapDataPacketHandler.class).info("Saved tile " + (rx + i) + ","+ (ry + j));
                        }
                    }
                }
            }
