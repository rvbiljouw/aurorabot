...
public class RSPathFinder implements Runnable {
    ...
    private void reload() {
        int plane = Context.get().getClient().getPlane();
        RSRegion region = new RSRegion(Context.get().getClient().getRegions()[plane].getClippingMasks());
        pathFinder = new AStarPathFinder(region, 100, true, new ClosestHeuristic());
    }

    public Path getPath(int destX, int destY, int full) {
        reload();
        Point destPoint = getFixed(new Point(destX - Context.get().getClient().getBaseX(), destY - Context.get().getClient().getBaseY()));
